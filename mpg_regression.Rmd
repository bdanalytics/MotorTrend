...
1. Test other regression techniques (e.g. additive models in glm) to better quantify the relationship.
2. Add Maserati labels to points in scatter plots to better understand it's influence on the model
3. ???
...
build_fit_plot_df <- function(fit_df) {
    fit_plot_df <- melt(fit_df[, c("id", "mpg", "mpg.predict", "mpg.residuals", 
...
#all.equal(mpg_fit_plot_old_df, mpg_fit_plot_df)
...
The Maserati Bora is the most influential point and without it manual transmission would be ``r format(abs(dfbeta(mpg_wt_i_fit)[max_hatval_car, ]["am_fctrmanual"]), digits=2)`` mpg lower.

```{r q2_print_model_wt_i}
build_fit_formula <- function(fit) {
    ...
    #for (row in seq(1:2))        
        coeff_str <- paste0(coeff_str, ifelse((row == 1), "", " + "), 
                            ...
                            ifelse((row == 1), "", " * "), regressors[row])                            
    print(coeff_str)
    str <- paste0(tmp_str[2], " ", tmp_str[1], " ", coeff_str)
    return(str)
}
display_fit_frmla <- build_fit_formula(mpg_wt_i_fit)
print(display_fit_frmla)
print(confint(mpg_wt_i_fit))
```

The proposed model is ``r display_fit_frmla`` where `am_fctr` is 0 for manual transmission [1 for automatic], `wt` is weight (lb/1000) and `am_fctr:wt` is 0 for manual transmission [wt for automatic].
